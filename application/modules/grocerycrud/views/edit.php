	...
	foreach ($fields as $field) {
		...
		if ($input_fields[ $field->field_name ]->crud_type != 'relation_invisible') {
			if (
				...
				?>
					<div class="form-group" id="<?php echo $field->field_name; ?>_field_box">
						<div class="input-group"> <span class="input-group-addon" id="basic-addon1"> <?php echo $input_fields[$field->field_name]->display_as;?> <?php echo ($input_fields[$field->field_name]->required)? " <span class='required label label-danger'>*</span> " :
							""; ?> </span>
							<?php echo $input_fields[$field->field_name]->input;?>
							<!--<input type="text" class="form-control" placeholder="<?php echo $field->display_as;?>" name="<?php echo $field->field_name;
				?>" aria-describedby="basic-addon1"> -->
						</div>
						<?php if( ! empty( $field->description ) ):?>
						<span class="help-block"><?php echo strip_tags( $field->description );?></span>
...
<script>
	...
	var grocery_errors      =   {
        fields         :   '<?php echo __( 'The form fields has an error. Please check it and try again.', 'grocerycrud' );?>'
    }
</script>
<script>
