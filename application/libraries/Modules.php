...
class Modules
{
        ...
        if (! file_exists($extraction_path . '/' . 'config.xml')) {
            ...
            while (false !== ($file = readdir($temp_dir))) {
                if (! in_array($file, array( '.', '..' ))) {
                    ...
                    // if ( $fileRawName == $upload_details[ 'upload_data' ][ 'raw_name' ]) { // if internal folder name is the same as the raw name
                    // }
                }
            }
        }
    ...
    **/
     ...
     * @return void
     */
    public static function buildAssets( $module_namespace )
    {
        $assets_path = PUBLICPATH . 'modules' . '/' . $module_namespace;
        // Copy Assets to
        if (is_dir($assets_path)) {
            // create assets folder
            if ( ! is_dir( MODULESPATH . '/' . $module_namespace . '/assets' )) {
                mkdir( MODULESPATH . '/' . $module_namespace . '/assets' );
            }
            SimpleFileManager::copy( $assets_path, MODULESPATH . '/' . $module_namespace . '/assets' );
        }
    }
}
