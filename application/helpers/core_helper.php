...
function module_config( $module, $config ) {
    $path       =   explode( '.', $config );
    get_instance()->load->module_config( $module, $path[0] );
    return get_instance()->config->item( $path[1] );
}
 ...
 */
if ( ! function_exists( 'tendoo_debug' ) ) {
    function tendoo_debug( $object, $name = 'file-debug.json' ) {
        if ( ENABLE_DEBUG ) {
