...
<body class="login-page">
    <div class="login-box">
        <div class="register-logo"> <a href="<?php echo base_url();?>"><b>
                    <?php echo __('Tendoo CMS');?></b>
                <?php echo get('str_core');?></a> </div>
        <div class="login-box-body">
            <form method="post">
                <h3 class="page-header text-center">
                    <?php echo __('Permission request');?>
                </h3>
        ...
        $showButtons    =    false;
            if ($scopes && @$_GET[ 'cb' ] != null) {
                if (is_array($scopes)) {
					// var_dump( $scopes );
                    foreach ($scopes as $namespace => $scope) {
                        ?>
                <input type="hidden" value="<?php echo @$namespace;?>" name="scopes[]">
                <div class="info-box <?php echo @$scope[ 'color' ];
                        ?>"> <span class="info-box-icon"><i
                            class="<?php echo @$scope[ 'icon' ];
                        ?>"></i></span>
                ...
                <button type="submit" class="btn btn-success">
                    <?php _e('Allow');?></button>
                <button type="submit" name="refuse" value="true" class="btn btn-default">
